...
// 安装laravel环境*****************************
    ...
    composer create-project --prefer-dist laravel/laravel hy 5.5.*
...
// 数据库迁移************************************
    ...
    // 创建表 : 
        #1.创建表文件 : 
            命令行执行 : php artisan make:migration create_role_table  #将在database/migrations文件夹生成文件
...
// 数据库填充****************************
    //步骤:
        ...
        #2.修改run方法 : 
            public function run(){
                for ($i=0; $i <100 ; $i++) { 
                    DB::table('stus')->insert(['name'=>"n_{$i}",'user_id'=>1]);
                }
            }                                       
        #3.执行命令
...
//11.登录
    ...
    #2. LoginController里新增login方法如下 : 
        ...
        public function login(Request $request){
            if($request->isMethod('post')){
                $arr = array('email'=>$request->get('email'), 'password'=>$request->get('password'));
                if (Auth::attempt($arr)) {
...
通用语句：DB::statement('drop table users');
@查询构造器
%我们使用get方法获取表中所有记录
 $users = DB::table('users')->get();
%如果你只是想要从数据表中获取一行数据，可以使用first方法
$user = DB::table('users')->where('name', 'John')->first();
%不需要完整的一行，可以使用value方法从结果中获取单个值
$email = DB::table('users')->where('name', 'John')->value('email');
%获取包含单个列值的数组，可以使用pluck方法
$titles = DB::table('roles')->pluck('title');
%查询构建器还提供了多个聚合方法，如count, max, min, avg和 sum，你可以在构造查询之后调用这些方法：
$users = DB::table('users')->count();
$price = DB::table('orders')->max('price');
%使用select方法，你可以为查询指定自定义的select子句：
...
$users = DB::table('users')
                    ...
                    ->get();
%更多where子句                                        
whereBetween,whereNotBetween,whereIn,whereNotIn,whereNull方法验证给定列的值为NULL,whereNotNull方法验证给定列的值不是NULL,whereDate('created_at', '2016-10-10')方法用于比较字段值和日期,whereMonth,whereDay,whereYear,whereColumn方法用于验证两个字段是否相等,whereColumn('updated_at', '>', 'created_at'),
$users = DB::table('users')
            ->whereColumn([
